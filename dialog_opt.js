...
(function(){
  ...
  var NPC_TOPIC = {
    aqing: {
      weather: {2:["雨后的竹叶能止血，你记得。"],4:["雨停了，草还湿着。我刚采的。"]},
  ...
  /* 增强 chatWithNpc */
  var _chat = typeof chatWithNpc==="function" ? chatWithNpc : null;
  window.chatWithNpc = function(player, npc, topicId){
    ...
    var youSaid = topic.name || "交谈";
    pushChat(player, npc.id, "you", "（"+youSaid+"）");
  ...
  /* 增强送礼反馈 */
  var _gift = typeof giftNpc==="function" ? giftNpc : null;
  window.giftNpc = function(player, npc){
    ...
    var newFav = typeof getFavor==="function" ? getFavor(player,npc.id) : fav+d;
    var tier = tierOf(newFav);
    var reply = pick(GIFT_LINES[tier] || GIFT_LINES[0]);
    /* NPC 个性 */
    if(npc.id==="aqing" && tier>=3) reply = pick(["……我收下了。草还湿着。","叶哨我重新做了一枚，给你。"]);
    if(npc.id==="jian_tong" && tier>=3) reply = pick(["这、这我怎么好意思！","我给你看剑法！作为回礼！"]);
  ...
  function renderChatHistory(player, npcId){
    ...
    recent.forEach(function(m){
      if(m.role==="you"){
        html += '<div class="chat-line you"><span class="chat-tag">你</span> '+m.text+'</div>';
      }else{
        html += '<div class="chat-line npc"><span class="chat-tag">对方</span> 「'+m.text+'」</div>';
      }
    });
    html += "</div>";
    return html;
  ...
  /* 重写 modalNpc 对话体验 */
  var _modalNpc = typeof modalNpc==="function" ? modalNpc : null;
  ...
  window.modalNpc = function(npcId, opts){
    ...
    if(!npc){
      if(_modalNpc) return _modalNpc(npcId);
      return;
    }
    ...
    topics.forEach(function(t){
      ...
      var tip = locked ? ("需好感"+t.need) : ("+"+(t.favor||2));
      topicHtml += '<button class="btn sm chat-topic" data-id="'+t.id+'" title="'+tip+'" '+dis+'>'+
        t.name+(locked?("<span class=\"small\">·锁</span>"):"")+'</button>';
    });
    ...
    if(canRomance){
      if(bonded){
        ...
        pledgeHtml = '<button class="btn primary" id="pledgeBtn" style="margin:6px 0">私定终身</button>';
      }else{
    ...
    openModal(
      '<div class="modal-head">'+
        '<h2 class="section-title">'+(av?av+" ":"")+npc.name+' <span class="tag">'+npc.title+'</span>'+bondTag+'</h2>'+
    ...
    document.querySelectorAll(".chat-topic").forEach(function(el){
      el.onclick = function(){
        ...
        modalNpc(npcId, {
          line: r.line,
          flash: true,
          note: "好感 +"+r.gain+" · 今日交谈剩余 "+Math.max(0,chatLeft-1)+" 次"
        });
      };
    });
    ...
    if(pledgeBtn) pledgeBtn.onclick = function(){
      ...
      if(typeof saveGame==="function") saveGame(state);
      modalNpc(npcId, { line: r.ok ? (r.msg||greeting) : greeting, note: r.ok?"缘定今生":"", flash: r.ok });
    };
    ...
    document.querySelectorAll(".npc-act").forEach(function(el){
      el.onclick = function(){
        ...
        if(typeof saveGame==="function") saveGame(state);
        /* 互动后刷新，气泡仍用分层对话 */
        var line2 = typeof getDialogueLine==="function" ? getDialogueLine(npc, getFavor(p,npc.id)) : greeting;
        modalNpc(npcId, { line: line2, note: r.ok ? r.msg : r.msg, flash: r.ok });
      };
    });
  };
  ...
  if(!document.getElementById("dialog-opt-style")){
    ...
    st.textContent = [
      ...
      ".chat-tag{display:inline-block;font-size:10px;padding:0 4px;border:1px solid #4b3a2d;",
      "  color:#b9a58a;margin-right:4px}",
