...
(function(g){
  ...
  g.getPlayerRank=function(p){return p?rankByScore(scoreOf(p)):RANKS[0];};g.getPlayerRankName=function(p){return g.getPlayerRank(p).name;};g.getPlayerRankScore=function(p){return p?scoreOf(p):0;};
  ...
  function registerHooks(){
    if(installed||!g.Game||typeof Game.hook!=="function")return;
    installed=true;
    Game.hook("canLearn",function(next){return function(player,skill){var r=next.apply(this,arguments);if(r&&r.ok===false)return r;return gate(player,skill);};});
    ...
    Game.hook("openModal",function(next){return function(){var r=next.apply(this,arguments);setTimeout(function(){var p=g.state&&g.state.player;if(p)injectPanel(p);},0);return r;};});
    Game.on("player:ensure",function(p){g.syncPlayerRank(p);});Game.on("render:after",function(p){if(p)injectHud(p);});Game.on("city:explore",function(e){if(e&&e.player)g.syncPlayerRank(e.player);});
  }
  ...
  function ready(){registerHooks();var p=g.state&&g.state.player;if(p)g.syncPlayerRank(p);}
