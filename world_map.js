/** 两层地图：州府 + 江湖节点。第1步：开封/洛阳/扬州/杭州。话本借宋世地名，实指大雍山河 */
(function(){
  var CITIES={
    kaifeng:{id:"kaifeng",name:"开封（东京）",short:"开封",type:"州府",tier:"city",region:"中原",icon:"京",
    ...
    luoyang:{id:"luoyang",name:"洛阳（西京）",short:"洛阳",type:"州府",tier:"city",region:"中原",icon:"洛",
    ...
    yangzhou:{id:"yangzhou",name:"扬州",short:"扬州",type:"州府",tier:"city",region:"淮扬",icon:"扬",
    ...
    hangzhou:{id:"hangzhou",name:"杭州（临安）",short:"杭州",type:"州府",tier:"city",region:"两浙",icon:"杭",
  ...
  function injectCityMaps(){
    ...
    Object.keys(CITIES).forEach(function(cid){
      var c=CITIES[cid];
      if(maps.some(function(m){return m.id===cid;}))return;
  ...
  window.travelCity=function(player,toCityId){
    ...
    if(!edge)return{ok:false,msg:"无直达官道/运河，请改道"};
    ...
    var clock=typeof formatClockShort==="function"?formatClockShort(player)+" · ":"";
    var kindLab=edge.kind==="canal"?"漕运":"官道";
    var msg=clock+"经"+kindLab+(edge.label?("「"+edge.label+"」"):"")+"，赶路"+cost+"程，抵达【"+dest.name+"】。";
  ...
  function renderCityPanel(player){
    ...
    var children=(c.children||[]).map(function(id){
    ...
    var neigh=getCityNeighbors(c.id).map(function(n){
      ...
      return '<button class="btn sm city-travel" data-id="'+n.id+'">赴'+lab+'（'+n.cost+'程·'+(n.kind==="canal"?"漕运":"官道")+'）</button>';
    }).join("");
    return '<div class="city-panel"><div class="loc-head"><h2 class="section-title">'+c.icon+' '+c.name+'</h2><span class="tag">州府</span></div><p class="small">'+c.desc+'</p><p class="small" style="color:var(--gold)">话本借宋世地名，实指大雍山河。</p><h3 class="section-title" style="font-size:13px;margin-top:8px">下辖江湖</h3><div class="row compact-block">'+children+'</div><h3 class="section-title" style="font-size:13px;margin-top:8px">长途</h3><div class="row compact-block">'+neigh+'</div><p class="small">城际耗时更长；下辖地点出城后仍走原有山河网。</p></div>';
  }
  function injectWorldChrome(player){
    ...
    var wrap=locPanel.querySelector(".xmap-wrap, .pixel-map");
